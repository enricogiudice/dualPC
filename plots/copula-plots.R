...
for (n in c(50, 100, 150, 200)) {
 ...
 for (jj in 1:3) { 
  ...
  results_df %>% group_by(n, sig, correct, method, comparison, value) %>%
  ...
  compy <- "cpdag"
    ...
    ggplot() + 
      ...
      geom_path(data = summarised_results_df %>% 
                ...
                size = 1) + facet_grid(correct ~ sig, 
                     ## labeller=labeller(n = add_facet_text, df = add_facet_text2, .multi_line = FALSE)) + 
      labeller = label_bquote(cols = lambda==.(as.numeric(as.character(sig))))) + 
    ...
    ggsave(paste0("copula_parents_", exp_parents, "_n_", n, ".png"), width=7, height=3.75, dpi = 200)
